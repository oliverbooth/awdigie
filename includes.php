...
function imagezoom()
{
    ...
    if ("$zm[0]" !== "0") {
        $orimage = @imagerotate($orimage, $zm[0], 0);
    }
...
function imagetext()
{
    ...
    if ($dbtexten == "yes") {
        ...
        if (preg_match("/\\.txt/i", $dbtext)) {
            ...
            if (($txttext = @file($stxting)) == false) {
                $error = $error . "-CANT OPEN FILE: $stxting \n";
            }

            if ($brtxtdata[0] == "alllines") {
                $dbtext = join($txttext);
            }
            if ($brtxtdata[0] == "onlyline") {
                $dbtext = $txttext[($brtxtdata[1] - 1)];
            }
            if ($brtxtdata[0] == "last") {
                $dbtext = '';
                for ($ln = (sizeof($txttext) - $brtxtdata[1]); $ln < sizeof($txttext); $ln++) {
                    $dbtext = $dbtext . $txttext[$ln];
                }
            }
        }
        ...
        if ((@imagettftext($image, $dbtextsize, $rota, $t_pos[0], $t_pos[1], @imagecolorallocate($image, $rgb[0], $rgb[1], $rgb[2]), "fonts/$dbfont", textfilter($dbtext, $dbthisshow))) == false) {
            $error = $error . "imagettftext error in includes.php";
        }
    }
}
...
function watermark()
{
    ...
    if ($enabwater == "yes" or $dbwateren == "yes") {
        if (preg_match("/\\.txt/i", $dbwater)) {
            if (($wfilet = @file("$dbwater")) == false) {
                $error = $error . "-WATEMARK TXT IS NOT VALID: $dbwater \n";
            }
            $dbwater =  preg_replace("/\n/", "", $wfilet[(time() / 10) % sizeof($wfilet)]);
        }
        if (($png = @imagecreatefrompng("$dbwater")) == false) {
            $error = $error . "-CANT USE WATEMARK: $dbwater \n";
        }
