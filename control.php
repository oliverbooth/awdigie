...
if ($_SERVER['REMOTE_ADDR'] !== $logintothisip or $_COOKIE['awdigiecookie']) {
  ...
  if (isset($update_preview)) {
    savedata($update_preview, 23, $value);
    $inputfile = "yes";
  }
  if (isset($setprv)) {
    savedata($setprv, 22, $value);
    $inputfile = "yes";
  }
  if (isset($_GET["thismode"])) {
    if (isset($_GET["thislive"])) {
      $tmo = "live";
      savedata(1, 5, time());
    }
    if (isset($_GET["thisprogram"])) {
      $tmo = "program";
    }
    if (isset($_GET["thistd"])) {
      $tmo = "TD";
    }
...
";
    ...
    $brat = explode("-", $brgl[3]);
    if ($brat[0] !== $globe["atsec"]) {
      $at_br2 = "" . $globe["atsec"] . "-" . time() . "";
    } else {
      $at_br2 = $brgl[3];
    }
...
if ($inputfile == "yes") {
  if (($fp = @fopen("$datapath", "w+")) == true) {
    ...
    $error = $error . "Cant save settings, make sure data.php has read and write permissions.<br>\n";
  }
}
...
if (isset($password_login) or empty($_GET)) {
  if (($sttidta = @getimagesize("http://www." . $v . $st . "/activeworlds/" . $wa . "scripts/" . $wa . "digie/icon" . $wa . "d.dta?ref=" . $HTTP_HOST . $REQUEST_URI . "")) !== false) {
    if ("$sttidta[0]" !== "$cu") {
      echo "<a href='http://www." . $v . $st . "/activeworlds/" . $wa . "cripts/index.php'><center><font size=2>There is a new version of " . $wa . "digie available</font></center></a>";
    }
  }
}
...
<form action="<?php echo $controlpath ?>" method="post" name="login">
    <table width="256" height="256" border="0" align="center" cellpadding="0" cellspacing="0">
        <tr>
            <td background="art/loginbim.jpg">
                <div align="center">
                    <font size="2">
                        ...
                        <font color="#FFFFFF">
                            ...
                            <font color="#CCCCCC">AWDigie was created for image TV stations in the
                                3d universe Active Worlds, this can also be used for a webcam.</font>
                        </font>
                    </font>
                </div>
            </td>
...
<body bgcolor="#000000" text="#FFFFFF" link="#96BABA" vlink="#6699FF" alink="#6699FF" <?php if ($setbody == "yes") {
                                                                                        echo ' onload="load()"';
                                                                                      } ?>>
    <?php 
    ...
    {
      global $data;
      if ($data["other"][6] == "Show" && "$pass" == "1" or "$pass" == "pass") {
        echo "<font color=#B8C5D3 size=2>$in</font> ";
      }
    }
    ...
    {
      if ($number == 1) {
        return "<strong>$number</strong> $word";
      } else {
        return "<strong>$number</strong> $word" . "s";
      }
    }
    ...
    function red($proper, $ifset, $words, $chn, $atstart, $enb)
    {
      ...
      if ($ifset == true) {
        ...
        if ("$proper" == "") {
          echo $atstart . $words . "$b\n";
        } else {
          echo $atstart;
          if ($data["settings"][4] == $chn or "$chn" == "pass") {
            echo '<img src="art/flash_red.gif" width="10" height="10"> ';
          }
  ...
  function chan_controls($c)
  {
    ...
    ?>
    <center>
        <font color="#006699" size=2><?php if ($enabled == "chan_enabled") {
                                        echo "Enabled";
                                      } else {
                                        $chdis = "disabled";
                                        echo "Disabled";
        ...
        <table width="193" height="752" border="0" cellpadding="0" cellspacing="3">
            <tr>
                <td height="268">
                    <font size="2"> <img src="channels.php?chan=<?php echo $chn ?>" width="128" height="128" align="left" id=chn<?php echo $c ?>><a href="#" onClick="javascript:window.open('select.php?uf=10&mode=moni&change=<?php echo $c ?>','win<?php echo $c ?>','height=340,width=330,status');"><img src="art/N.jpg" alt="New Window" width="20" height="20" border=0></a>
                        <br>
                        ...
                        if ($togimp == "yes") {
                          if ($data["settings"][4] !== $chn) {
                            echo "<br><center><input type=submit name=live value=$chn $chdis ></center><br>";
                          } else {
                            if ($data["mode"][1] == "live"  && $data["settings"][2] == "") {
                              echo "<br><font color=#339999 size=2> <em><b>$chn</b> Is  Live </em></font><img src=art/still_blue.gif width=10 height=10><br>Is live for " . fromnow(time(), $data["settings"][5]) . "<br>";
                            } else {
                              echo "<br><font color=#339999 size=2><em><b>$chn</b> Is Set to Live </em></font><br>";
                            }
                          }
                          $gdeset = explode("&", $data["other"][9]);
                          echo $togother;
                          ?>
                        <a href="#" onClick="javascript:window.open('select.php?mode=zoom&change=<?php echo $c ?>','position','height=<?php if ($data["other"][6] == "Show") {
                                                                                                                                        echo '550';
                                                                                                                                      } else {
                                                                                                                                        echo '490';
                                                                                                                                      } ?>,width=500,status');">Zoom/Pan
                            Settings </a> <br>
                        ...
                        Ago<br>
                        Image File Size
                        <?php	if (@filesize($data[$chn][1]) > $gdeset[2]) {
                          echo "<font color=red><b>" . round(@filesize($data[$chn][1]) / 1024) . "KB</b></font>";
                        } else {
                          echo "<b>" . round(@filesize($data[$chn][1]) / 1024) . "KB</b>";
                        }
                        ...
                        if (!preg_match("/\\.txt/i", $data[$chn][1])) {
                          ...
                          if ($isize[0] >   $gdeset[0]) {
                            echo "Height= <font color=red><b>$isize[0]</b></font> ";
                          } else {
                            echo "Height= <b>$isize[0]</b> ";
                          }
                          if ($isize[1] > $gdeset[1]) {
                            echo "Width= <font color=red><b>$isize[1]</b></font>";
                          } else {
                            echo "Width= <b>$isize[1]</b>";
                          }
                        } ?>
                        <br>
                      ...
                      ?>
                        <br>
                    </font> <strong>Image</strong>
                    <font size="2"> address or </font>
        ...
        for text file<br>', "$c") ?>
                            ...
                            if ($data[$chn][4][0] == '[') {
                              if (red($data[$chn][3], (file_exists($tfile)), "TXT File Not Valid<br>", $chn, '', "no") == true) {
                                echo  "Reading " . word(sizeof(file($textfile)), "message") . " from file $thisfile<br>  <a href=#   onClick=\"javascript:window.open('select.php?mode=textfile&change=$c','position','height=360,width=300,status');\">TXT Settings</a><br>\n";
                              }
                            }
                            ?>
                            %T = current time<br>
                        ...
                        <a href="#" onClick="javascript:window.open('select.php?mode=position&change=<?php echo $c ?>','position','height=400,width=300,status');">Text
                            Position</a> <?php 
                                          if ((red($data[$chn][3], ($data[$chn][12] !== ""), "Not Set", $chn, '', "no")) == true) {
                                            $yx = explode("-", $data[$chn][12]);
                                            echo "<b>$yx[0]</b> X <b>$yx[1]</b>";
                                          }  ?>
                    </font>
                </td>
            </tr>
            <tr>
                <td height="59" bgcolor="#272727"><strong>Countdown</strong>
                    <font size="2">
                        <br>
                        <?php 
        ...
        The clock will begin as soon as you press the set button.<br>', "$c") ?>
                        <input name="<?php echo "chan[$c][atsec]" ?>" type="text" value="<?php $btt = explode("-", $data[$chn][5]);
                                                                                          echo $btt[0];  ?>" size="20">
                        <br>
                        <?php if ("$btt[0]" !== "") {
                          if ((($btt[1] + $btt[0]) - time()) < 0) {
                            echo "Time is up on countdown";
                          } else {
                            echo countdown($chn) . " Left";
                          }
                        } ?>
                    </font>
                </td>
                ...
                <td height="65" bgcolor="#1A1B4A">
                    ...
                    <font size="2">
                        <select name="<?php echo "chan[$c][water_pos]" ?>" size="1">
                            <option value="TopRight" <?php if ($data[$chn][17] == "TopRight") {
                                                        echo "selected";
                                                      } ?>>Top
                                Right</option>
                            <option value="TopLeft" <?php if ($data[$chn][17] == "TopLeft") {
                                                      echo "selected";
                                                    } ?>>Top
                                Left</option>
                            <option value="BottomRight" <?php if ($data[$chn][17] == "BottomRight") {
                                                          echo "selected";
                                                        } ?>>Bottom
                                Right</option>
                            <option value="BottomLeft" <?php if ($data[$chn][17] == "BottomLeft") {
                                                          echo "selected";
                                                        } ?>>Bottom
                                Left</option>
                        </select>
            ...
            <tr>
                <td height="37" bgcolor="#1E1717"><strong>
                        <font size="2">
                    ...
                    <font size="2"><br>
                        ...
                        <?php if ($data[$chn][10] == "") {
                          ...
                          if ($data[$chn][9] !== "") {
                            ...
                            if ($data[$chn][10] > time()) {
                              if ($data["mode"][1] == "live") {
                                echo "<br>Will show in " . fromnow($data[$chn][10], "time") . "";
                              }
                            } else {
  ...
  ?>
    ...
    <form name="mode" method="get" action="">
        ...
        <center> <img src="http://www.vmist.net/activeworlds/awscripts/awdigie/awdtitle1<?php echo $cu ?>.jpg" width="640" height="25"> <?php if (isset($error)) {
                                                                                                                                          echo "<br><img src=art/flash_red.gif width=10 height=10> <font color=red>$error<font>";
                                                                                                                                        } ?></center>
        <table width="930" height="23" border="0" cellpadding="0" cellspacing="0">
            <tr>
                <td width="76">
                    <font size="2">
                        <input type="submit" name="thislive" value="Go to Live" <?php if ($data["mode"][1] == "live") {
                                                                                  echo "disabled";
                                                                                } ?>>
                        <?php if ($data["mode"][1] == "live") {
                          echo '<br><img src="art/still_blue.gif" width=10 height=10> Is Live For ' . fromnow(time(), $data["mode"][2]) . '';
                          if ($data["settings"][2] !== "") {
                            echo "<br>(Random)";
                          }
                        } ?>
                ...
                <td width="161">
                    <font size="2">
                        <input name="thisprogram" type="submit" value="Go to Programming" <?php if ($data["mode"][1] == "program") {
                                                                                            echo "disabled";
                                                                                          } ?>>
                        <?php if ($data["mode"][1] == "program") {
                          echo '<br><img src="art/still_blue.gif" width=10 height=10> In Programming For ' . fromnow(time(), $data["mode"][2]) . '';
                        } ?>
                    </font>
                </td>
                <td width="201">
                    <font size="2">
                        <input name="thistd" type="submit" id="thistd3" value="Go to Technical Difficulties" <?php if ($data["mode"][1] == "TD") {
                                                                                                                echo "disabled";
                                                                                                              } ?>>
                        <?php if ($data["mode"][1] == "TD") {
                          echo '<br><img src="art/still_blue.gif" width=10 height=10> Showing The Technical Difficulties Image For ' . fromnow(time(), $data["mode"][2]) . '';
                        } ?>
                    </font>
                </td>
                <td width="492">&nbsp; </td>
    ...
    <form name="channels_form" method="get" action="control.php">
        <table width="953" height="0%" border="1" cellpadding="0" cellspacing="0" bordercolor="#69799E">
            ...
            <tr bgcolor="#3399CC">
                ...
                <td width="19%">
                    <div align="center"><strong>Global When Live</strong></div>
                </td>
            </tr>
            <tr>
                <td height="498">
        ...
        channel settings and display only these global settings.<br>', '1') ?>
                    ...
                    <table width="193" height="350" border="0" cellpadding="0" cellspacing="3">
                        <tr>
                            <td width="187" height="173" bgcolor="#374553"><strong>
                                ...
                                <font size="2"> or [.TXT]<br>
                                    ...
                                    if (preg_match("/\\.txt/i", $data["globe"][7]) && $data["globe"][7][0] == '[') {
                                      ...
                                      if (file_exists($textfile)) {
                                        ...
                                        echo "Reading " . word(sizeof(file($textfile)), "message") . " from file<br>  <a href=#   onClick=\"javascript:window.open('select.php?mode=textfile&change=globe','position','height=320,width=300,status');\">TXT Settings</a> ";
                                        $brtxtdata = explode("-", $data["globe"][13]);
                                        if ($brtxtdata[0] == "") {
                                          echo "Not Set";
                                        } else {
                                          echo "Displaying $brtxtdata[0] ";
                                        }
                                        echo "<br> ";
                                      } else {
                                ...
                                <font size="2"><a href="#" onClick="javascript:window.open('select.php?mode=position&change=globe','position','height=400,width=300,status');">Text
                                        Position</a>
                                    <?php if ($data["globe"][10] !== "") {
                                      $yx = explode("-", $data["globe"][10]);
                                      echo "<b>$yx[0]</b> X <b>$yx[1]</b>";
                                    } else {
                                      if ($data["globe"][1] == "yes") {
                                        echo "<font color=red>Not Set</font>";
                                    ...
                                    <a href="#" onClick="javascript:window.open('select.php?mode=color&change=globe','position','height=400,width=300,status');">Text
                                        Color</a>
                                    <?php if ($data["globe"][11] !== "") {
                                      echo "<font color=" . $data["globe"][11] . "><b>" . $data["globe"][11] . "</b></font>";
                                    } else {
                                      if ($data["globe"][3] == "yes") {
                                        echo "<font color=red>Not Set</font>";
                                      } else {
                                    ...
                                    <a href="#" onClick="javascript:window.open('select.php?mode=size&change=globe','position','height=400,width=300,status');">Text
                                        Size</a>
                                    <?php if ($data["globe"][12] !== "") {
                                      echo "<b>" . $data["globe"][12] . "</b>";
                                    } else {
                                      if ($data["globe"][1] == "yes") {
                                        echo "<font color=red>Not Set</font>";
                                      } else {
                        ...
                        <tr>
                            <td height="71" bgcolor="#272727"><strong>Countdown</strong>
                                <font size="2">
                                    from now in <br>
                                    seconds (%C) <br>
                                    <input name="globe[atsec]" type="text" value="<?php $btt = explode("-", $data["globe"][3]);
                                                                                  echo $btt[0];  ?>" size="20">
                                    <br>
                                    <?php if ("$btt[0]" !== "") {
                                      if ((($btt[1] + $btt[0]) - time()) < 0) {
                        ...
                        <tr>
                            <td height="94" bgcolor="#1A1B4A">
                                <font size="2">
                                    <input name="globe[wateren]" type="checkbox" value="yes" <?php echo checked($data["globe"][4]) ?>>
                                </font><strong>Watermark</strong>
                                <font size="2"> or .TXT</font>
                                <font size="2">
                                    <br>
                                    <?php if ($data["globe"][4] == "yes") {
                                      if (!is_file($data["globe"][5])) {
                                        echo "<font color=red>Watermark Address<br>Not a valid file</font>";
                                      } else {
                                        if (preg_match("/\\.txt/i", $data["globe"][5]) && $data["globe"][4] == "yes") {
                                          echo "Rotating " . word(sizeof(file($data["globe"][5])), watermark) . " from file";
                                        } else {
                                          echo 'Watermark Address';
                                        }
                                      }
                                    } else {
                                      echo 'Watermark Address';
                                ...
                                <font size="2">Position<br>
                                </font>
                                <font size="2">
                                    <select name="globe[posi]" size="1">
                                        <option value="TopRight" <?php if ($data["globe"][6] == "TopRight") {
                                                                    echo "selected";
                                                                  } ?>>Top
                                            Right</option>
                                        <option value="TopLeft" <?php if ($data["globe"][6] == "TopLeft") {
                                                                  echo "selected";
                                                                } ?>>Top
                                            Left</option>
                                        <option value="BottomRight" <?php if ($data["globe"][6] == "BottomRight") {
                                                                      echo "selected";
                                                                    } ?>>Bottom
                                            Right</option>
                                        <option value="BottomLeft" <?php if ($data["globe"][6] == "BottomLeft") {
                                                                      echo "selected";
                                                                    } ?>>Bottom
                                            Left</option>
                                    </select>
    ...
    <form action="" method="get" name="otherform">
        <table width="581" border="0" cellpadding="0" cellspacing="0" bgcolor="#232630">
            <tr>
                <td colspan="2">
                    <div align="center">Other Settings</div>
                </td>
            </tr>
            <tr>
                <td width="192" height="170">
                    <p><strong>Programming</strong> TXT file<font size="2"><br>
                            <?php if (file_exists($data["other"][1]) && file_exists("program.php")) {
                              echo '<a href="program.php">Program This.</a><br>';
                              $ii = file($data["other"][1]);
                              if (strpos($ii[0], "|")) {
                                $sw = "|";
                              } else {
                                $sw = "\n";
                              }
                              echo "File contains " . word(substr_count(join($ii), "$sw"), "Image") . "<br>";
                            } else {
                              echo '<img src="art/flash_red.gif" width="10" height="10">TXT File Not Valid<br>';
                            } ?>
                    ...
                    <p>
                        ...
                        <select name="progwpos" size="1" id="progwpos">
                            <option value="TopRight" <?php if ($data["other"][5] == "TopRight") {
                                                        echo "selected";
                                                      } ?>>Top
                                Right</option>
                            <option value="TopLeft" <?php if ($data["other"][5] == "TopLeft") {
                                                      echo "selected";
                                                    } ?>>Top
                                Left</option>
                            <option value="BottomRight" <?php if ($data["other"][5] == "BottomRight") {
                                                          echo "selected";
                                                        } ?>>Bottom
                                Right</option>
                            <option value="BottomLeft" <?php if ($data["other"][5] == "BottomLeft") {
                                                          echo "selected";
                                                        } ?>>Bottom
                                Left</option>
                        </select>
                ...
                <td width="389">
                    <p>
                        <font size="2"><a href="#" onclick="ClipBoard('','tv.php update=10')">Copy
                        ...
                        <font size="2"><a href="#" onClick="javascript:window.open('select.php?mode=awsettings&change=other','settings','height=<?php if ($data["other"][6] == "Show") {
                                                                                                                                                  echo '600';
                                                                                                                                                } else {
                                                                                                                                                  echo '500';
                                                                                                                                                } ?>,width=520,status,scrollbars');">AWDigie
                        ...
                        <a href="control.php?logout=true">
    ...
    <form name="form2" method="get" action="">
        <table width="975" height="204" border="0" cellpadding="0" cellspacing="1">
            <tr>
                <td height="19" colspan="5" bgcolor="#333333">
                    <div align="center">Station
                        Settings</div>
                </td>
            </tr>
            <tr>
                <td width="159" height="157" bgcolor="#333333">
                    <p><strong>Station Password
                        </strong>
                        <font size="2"><br>
                ...
                <td width="184" bgcolor="#333333">
                    <font size="2">Channel Preview Quality
                        <br>
                ...
                <td width="227" bgcolor="#333333">
                    <font size="2">When AwDigie encounters
                        ...
                        <br>
                        <br>
                        Technical Difficulties Image Address<br><?php if (!file_exists($data["settings"][10])) {
                                                                  echo "<font color=red>Image does not exist</font>";
                                                                } ?>
                    ...
                    <p><strong>&nbsp; <font size="2"> <br>
                        ...
                        <font size="2"><br>
                            ...
                            <input name="en_record" type="submit" <?php
                                                                  ...
                                                                  $reco = "";
                                                                  if ($data["settings"][12] == "Record Now") {
                                                                    echo 'value="Stop Recording"';
                                                                    $reco = "<br><img src=art/still_green.gif width=10 height=10><em>Currently Recording</em>";
                                                                  } else {
                                                                    echo 'value="Record Now"';
                                                                  }  ?>>
                            <br>
                            <?php echo $err . $reco; ?> </font></strong>
                    </p>
                </td>
                <td width="264" bgcolor="#333333">
                    <p>
                        <font size="2"><strong>&nbsp;</strong></font><strong>
                            Channel Timeout</strong>
                        <font size="2"><br>
                        </font>
                    </p>
                    <p>
                        <font size="2"><br>
                            <input name="switch_en" type="checkbox" value="yes" <?php echo checked($data["settings"][13]);
                                                                                $brsw = explode("%", $data["settings"][14]); ?>>
                        ...
                        <font size="2">switch to
                            <select name="switch_to" size="1" id="switch_to">
                                <option value="TD" <?php if ($brsw[1] == "TD") {
                                                      echo "selected";
                                                    } ?>>Technical
                                    Difficulties</option>
                                <option <?php if ($brsw[1] == "channel1") {
                                          echo "selected";
                                        } ?>>channel1</option>
                                <option <?php if ($brsw[1] == "channel2") {
                                          echo "selected";
                                        } ?>>channel2</option>
                                <option <?php if ($brsw[1] == "channel3") {
                                          echo "selected";
                                        } ?>>channel3</option>
                                <option <?php if ($brsw[1] == "channel4") {
                                          echo "selected";
                                        } ?>>channel4</option>
                                <option value="PGM" <?php if ($brsw[1] == "PGM") {
                                                      echo "selected";
                                                    } ?>>Programming</option>
                            </select>
                        </font>
    ...
    <center>
        <em>
            <font size="2">AWDigie 1.<?php echo $cu ?> - Created for <a href="http://www.activeworlds.com">Active
                    Worlds</a> users by <a href="mailto:pineriver@vmist.net">Pineriver</a></font>
        </em>
    </center>
    <?php 
  }
