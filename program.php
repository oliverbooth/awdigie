    ...
    if (isset($_GET["driveimage"])) {
      ...
      function notes($in)
      {
        ...
        if ($data["other"][6] == "Show") {
          echo "<font color=\"#003366\" size=2>$in</font> ";
        }
      }
      ...
      {
        global $strlength;
        if (strlen($in) > $strlength) {
          return substr($in, 0, $strlength) . '..';
        } else {
          return $in;
        }
      }
      ...
      {
        if ($input == 1) {
          return "<strong>$input</strong> $word";
        } else {
          return "<strong>$input</strong> $word" . s;
        }
      }

      $DB = @file($datafile);
      if (!strpos($DB[0], "|")) {
        echo "<font size=2>You need to run <a href=\"";
        if (file_exists("txt_to_program.php")) {
          echo "txt_to_program.php";
        } else {
          echo "http://www.vmist.net/scripts/download.php?file=txt_to_program";
        }
        echo '">txt_to_program</a> in order to use AWD Program.</font>';
      } else {
          ...
          if (is_dir($imfield)) {
            ...
            while (false !== ($fl = readdir($dh))) {
              if ($fl !== '..' && $fl !== '.' && eregi(("\.jpg|\.jpeg"), $fl)) {
                if ($dfl) {
                  $dri_str = "$imfield/$fl^Folder - $imfield^None^$imfield^|";
                  unset($dfl);
                } else {
...
";
            ...
            if (!@imagecreatefromjpeg($imfield)) {
              $imfielderror = "Can't Use Image.<br>";
            } else {
...
<body>
    ...
    <?php echo "AWD Program - <strong>$datafile</strong>";
    notes("<br>Small program for easier editing the program guide."); ?>
    ...
    <form action="<?php echo $thispath  ?>" method="post" name="formlist" id="formlist">
        <table width="694" border="0" cellspacing="3" cellpadding="3">
            <tr>
                <td width="200" height="23" bgcolor="#F14B4B">
                    <font color="#FFFFFF" size="2">
                        ...
                        <input name="imfield" type="text" <?php if ($imfielderror) {
                                                            echo 'style="color:red"';
                                                          } ?> value="<?php echo $imfield ?>">
                ...
                <td width="273" rowspan="2" bgcolor="#3399FF">
                    <div align="left">
                        <font color="#FFFFFF" size="2"><?php echo word($total_images, Image) . " Total."; ?><br>
                            Estimated Runtime:
                            <?php	
                            if ($net[4] == "") {
                              $net[4] = 10;
            ...
            <tr>
                <td height="200" colspan="4">
                    <div style="height:100%; overflow:auto;" bgcolor="#FFFFFF">
                        ...
                        <table width="100%" height="34" border="0" cellpadding="0" cellspacing="0" bgcolor="#EFEFEF">
                            <tr>
                                <td width="12%">
                                    <font color="#000000" size="2">Place</font>
                                </td>
                                <td width="16%">
                                    <font color="#000000" size="2">Name</font>
                                </td>
                                <td width="21%">
                                    <font color="#000000" size="2">Path</font>
                                </td>
                                <td colspan="2">
                                    <font color="#000000" size="2">Type</font>
                                </td>
                                <td colspan="2">
                                    <font size="2">Plugin File</font>
                                </td>
                            </tr>
                            <?php 
                            $y = 0;
                            ...
                            <tr id=tabimage<?php echo $y ?>>
                                <td height="19" id=tabNum<?php echo $y ?>> <input type="checkbox" name="check[<?php echo $y ?>]" value="<?php echo $y ?>">
                                    ...
                                    </font> <input type="hidden" name="valnum<?php echo $y ?>" width="13" height="16" id="valnum<?php echo $y ?>" hidden value="<?php echo $y ?>">
                                </td>
                                <td id="tabName<?php echo $y ?>">
                                    <font size="2"><a id="name<?php echo $y ?>" onClick="rename(this.id,'Name',<?php echo $y ?>,null)"><?php echo short($br[1])  ?></a>
                                        <input type="hidden" name="valname[<?php echo $y ?>]" id="valname<?php echo $y ?>" value="<?php echo $br[1]  ?>">
                                    </font>
                                </td>
                                <td id="tabPath<?php echo $y ?>">
                                    <font size="2" <?php if (!@imagecreatefromjpeg($br[0]) && $images == 1) {
                                                      echo 'color="red"';
                                                      $imerror = y;
                                                    } else {
                                                      $tto = y;
                                                    }
                                                    if ($images == 1) { ?> onClick="rename('path<?php echo $y ?>','Path',<?php echo $y ?>,null)" <?php 
                                                                                                                                                                                                                                                          } ?>>
                                        <?php if ($images > 1) {
                                          echo "<img src=\"folder.gif\"  width=\"17\" height=\"13\" > ";
                                        } else {
                                          echo "<img src=\"jpeg.gif\"  width=\"13\" height=\"16\" > ";
                                        } ?>
                                ...
                                <td width="19%">
                                    <font color="#000000" size="2"><a href="#" onclick="remove.value='<?php echo $y ?>'; formlist.submit(); ">Remove</a>
                                        <?php if ($tto) { ?>
                                        <a href="#" onclick="load(<?php echo $y;
                                                                  if ($images !== 1) {
                                                                    echo ",'$thispath?driveimage=$br[3]'";
                                                                  } ?>)">Preview</a></font>
                                    <?php 
                                  } ?>
                                </td>
                                <td width="23%" id="tabPlug<?php echo $y ?>">
                                    <font size="2"><a id="plug<?php echo $y ?>" onClick="rename(this.id,'Plug',<?php echo $y ?>,'renamebox','Edit ')">
                                            <?php if ("$br[2]" == "") {
                                              $br[2]  = None;
                                            }
                                            echo short($br[2]) ?>
                                        ...
                                        <input type="hidden" id="filesize<?php echo $y ?>" value="<?php if ($images == 1) {
                                                                                                    echo  round(@filesize($br[0])  / 1200) . 'KB';
                                                                                                  }  ?>">

                                        <input type="hidden" id="imagesize<?php echo $y ?>" value="<?php if ($images == 1) {
                                                                                                      $s = @getimagesize($br[0]);
                                                                                                      echo "Height=$s[0] Width=$s[1]";
                                                                                                    } else {
                                                                                                      echo "$images Images";
                                                                                                    }  ?>">
                                    </font>
        ...
        <font size="2">
            ...
            <?php if ($imerror) {
              echo ' <font color="#FF0000" size="2">*Can\'t use image</font>';
            } ?>
            <br>
            With Selected
        ...
        <font size="2">
            <input name="showtime" type="checkbox" id="showtime" value="y" <?php if ($showtime) {
                                                                              echo checked;
                                                                            } ?> onclick="formlist.submit()">
