...
if ($data["mode"][1] == "TD") {
    ...
    if ($data["mode"][1] == "program") {
        if (($progf = fopen($data["other"][1], "r+"))  == false) {
            $error = $error . "-CANT OPEN PROGRAM FILE: " . $data["other"][1] . " \n";
        }
...
", "", $pimages));
            ...
            if (!$biir[1]) {
                $biir = choose(($numofcun - 1));
            }
            $dbaddess = $biir[0];
            if ("$biir[2]" !== "" && "$biir[2]" !== "None") {
                if ((@include($biir[2]))  == false) {
                    $error = $error . "-CANT USE IM PLUG: " . $biir[2] . " \n";
                }
            }
        } else {
            $progf = explode("\n", $pimages);
        ...
        if (($image = @imagecreatefromjpeg("$dbaddess")) == false) {
            $error = $error . "-CANT USE IMAGE: $dbaddess \n";
        }
    ...
    if ($data["mode"][1] == "live") {
        ...
        for ($t = 1; $t < 5; $t++) {
            if ($data["channel" . $t][9] == "yes" && $data["channel" . $t][10] < time()) {
                $ocurrent = $data["channel" . $t];
            }
        }
        ...
        if ($data["settings"][2] == "yes") {
            ...
            if ($data["settings"][6] == "yes") {
                if (($rotchn = @file($data["settings"][7])) == false) {
                    $error = $error . "-CANT USE CH ROTATE FILE\n";
                }
        ...
        if ($data["other"][7] !== "") {
            ...
            if ((@include($data["other"][7]))  == false) {
                $error = $error . "-CANT USE PLUGIN: " . $data["other"][7] . " \n";
            }
        }

        if ($data["overchan"] !== "") {
            $current = $data[$data["overchan"]];
        }
        if (preg_match("/\\.txt/i", $dbaddess)) {
            if (($tdf = @file("$dbaddess"))  == false) {
                $error = $error . "-CANT OPEN FILE: $dbaddess \n";
            }
    ...
    if ($dbfunction == "") {
        $dbfunction = "watermarkYimagetext";
        if ($data["globe"][8] !== "chan_enabled") {
            if ($data["mode"][1] !== "program") {
                $error = $error . "-ERROR READING DB FUNCTION STRING\n";
            }
        }
    }
...
");
    ...
    if ($data["settings"][8] == "yes" && $_COOKIE['awdigiecookie'] == $data["settings"][1] && "$error" !== "") {
        if ($image == "") {
            ...
            imagefilltoborder($image, 256, 200, 150, ImageColorAllocate($image, 0, 0, 0));
        }
    ...
    if (isset($status) && "$status" == "true") {
        echo "<body bgcolor=0 text=white>\n";
        if ($error) {
            echo "Station Errors<br>" . str_replace("\n", "<img src=art/flash_red.gif>\n<br>", $error);
        } else {
            echo "Station is up\n<br>Current Mode <b>" . $data["mode"][1] . "</b>";
        }
    } else {
